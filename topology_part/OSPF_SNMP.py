...
import binascii
import sys
   # Here we check if the necessary packages have been installed, if not we suggest installing them with providing the links.
try:
...
try:    
    #Module for terminal text coloring
...
try:
    print Style.BRIGHT +  "\n############ OSPF DISCOVERY TOOL ##############"
    print "Your device should be running OSPF in the network!"
    print "Devices should have the same SNMP community string \n"
    ip = raw_input(Fore.CYAN + Style.BRIGHT + "\n* Please enter root device IP: ")
...
def ip_is_valid():
    ...
    #Pinging the address, to check the machine connectivity
...
def snmp_get(ip):
    ...
    #cmdGen a command generator object
    ...
    #Adding OSPF data per device in the ospf_device dictionary
...
while True:
    try:
        ...
        print Fore.MAGENTA + Style.BRIGHT + "* Please choose an action:\n\n1 - Export OSPF devices to CSV file\n2 - Generate OSPF network topology\ne - Exit"
